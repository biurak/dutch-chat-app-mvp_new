  ...
  try {
    ...
    const systemMessage = {
      ...
      content: `
        ...
        2. For yes/no questions, ALWAYS provide exactly 3 suggestions:
    ...
    try {
      ...
      try {
        ...
        return {
          ai_reply: 'Ik heb een probleem met het verwerken van uw bericht. Kunt u het opnieuw proberen?',
    ...
    if (!result.ai_reply && !result.message) {
      console.error('AI response missing ai_reply/message. Full response:', JSON.stringify(result, null, 2));
      ...
      return {
        ai_reply: typeof possibleMessage === 'string' ? possibleMessage : 'Ik heb een probleem met het verwerken van uw bericht.',
    ...
    const suggestions = (result.suggestions || []).reduce<Array<{dutch: string, english: string}>>((acc, suggestion) => {
